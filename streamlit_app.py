...
if len(selected_players) < 10:
    ...
    if st.button("Show Prediction Explanation"):
        ...
        shap_explainer = shap.KernelExplainer(SVM_model.predict_proba, scaler.transform(df.drop(['Player'], axis=1).values))
        ...
        lime_explainer = LimeTabularExplainer(
            scaler.transform(df.drop(['Player'], axis=1).values),
